    ...
    <section id="process" className="section-padding border-y border-gray-200/70 bg-[linear-gradient(180deg,#f7f3eb_0%,#f5efe8_100%)]">
      <div className="w-full max-w-none px-3 sm:px-4 lg:px-6">
        ...
        <motion.div 
          ...
          className="relative mx-0 grid grid-cols-1 gap-3 auto-rows-fr items-stretch sm:grid-cols-2 sm:gap-4 md:grid-cols-2 lg:grid-cols-4 lg:gap-6"
        >
          ...
          {processSteps.map((step) => (
            <motion.div 
              ...
              className="group relative z-10 flex h-full min-h-0 flex-col overflow-hidden rounded-[1rem] border border-[#E9E4DC] bg-white p-4 text-[8.5px] shadow-[0_8px_20px_rgba(17,17,17,0.04)] transition-all duration-300 hover:-translate-y-1 hover:border-[#D71920]/25 hover:shadow-[0_16px_36px_rgba(17,17,17,0.08)] items-center text-center sm:p-4 sm:text-[9px] md:p-5 md:text-[10px] md:rounded-[1.25rem] lg:p-8 lg:text-left lg:text-sm"
            >
              <div className="absolute left-0 top-0 h-[3px] w-0 bg-[#D71920] transition-all duration-300 group-hover:w-full" />
              
              <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-[0.75rem] border border-[#E9E4DC] bg-white shadow-[0_4px_12px_rgba(17,17,17,0.04)] transition-all duration-300 group-hover:border-[#D71920]/40 group-hover:shadow-[0_8px_20px_rgba(215,25,32,0.14)] md:h-14 md:w-14 md:rounded-[1rem] md:mb-4 lg:h-20 lg:w-20 lg:rounded-[1.25rem] lg:mb-6">
                <span className="text-xs font-black text-gray-300 transition-colors duration-300 group-hover:text-[#D71920] md:text-2xl lg:text-3xl">{step.step}</span>
              </div>
              
              <div className="flex flex-col flex-1 justify-between">
                <h3 className="mb-2 text-[8.5px] font-black leading-[1.1] tracking-tight text-black transition-colors duration-300 group-hover:text-[#D71920] xs:text-[9px] sm:text-[10px] md:text-sm lg:mb-3 lg:text-lg">{step.title}</h3>
                <p className="text-[8px] font-light leading-[1.3] text-gray-500 xs:text-[8.5px] sm:text-[9px] md:text-xs lg:text-sm lg:leading-relaxed">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
